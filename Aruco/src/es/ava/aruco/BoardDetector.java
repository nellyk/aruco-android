...
import es.ava.aruco.Board.BoardConfiguration;
...
public class BoardDetector {
	...
	private float detect(Vector<Marker> detectedMarkers, BoardConfiguration conf, Board bDetected,
		...
		int[][] detected = new int[width][height];// indices of the markers
		...
		if(camMatrix.rows()!=0 && markerSizeMeters>0 && detectedMarkers.size()>1){
	        ...
	        for(int y=0;y<height;y++)
				...
				if(detected[y][x] != -1){
					...
					for(int p=0;p<4;p++){
						buffer[(currIndex+p)*2] = points.get(p).x;
						buffer[(currIndex+p)*2+1] = points.get(p).y;
					}
					imgPoints.put(0, 0, buffer);
		...
		return (nMarkInBoard/( conf.width*conf.height));
	}
